...
established empirically by Bell-inequality
experiments~\cite{Bell1964,Aspect1982,Zeilinger2023Nobel} and
articulated philosophically by relational quantum
...
Zeilinger's Nobel Lecture~\cite{Zeilinger2023Nobel} providing a
...
Sec.~\ref{sec:rct} depend only on the joint state $\rho_{SE}$ at
the time of evaluation, so the temporal ordering of prior interaction
...
Two distinct quantities are at play along this trajectory, and the
...
This is why $\bar{\alpha}$, not $S$-$E$ entanglement, is the appropriate
control parameter for the $\URA$ gate of Sec.~\ref{sec:gate}: the gate
must act on the coherence still locally available in $S$ before that
coherence is irretrievably distributed into the environment.
