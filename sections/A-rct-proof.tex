...
\textbf{Claim.} There is no value assignment
$v: \mathcal{O}(\mathcal{H}_S) \to \mathbb{R}$
mapping each self-adjoint observable $\hat{A}$ on $\mathcal{H}_S$ to a
definite eigenvalue $v(\hat{A}) \in \mathrm{spec}(\hat{A})$ prior to
relational interaction, consistently with all quantum predictions.
...
interpolating from the identity ($\theta = 0$) to the \textsc{swap}
gate ($\theta = \pi/2$), with maximum entanglement capability at
$\theta = \pi/4$ (the \textsc{iswap} point).
...
Gate decomposition: $\URA(\theta)$ compiled via \texttt{transpile} with
