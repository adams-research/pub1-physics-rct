...
and in the parameterization of $\URA$ (Sec.~\ref{sec:ura}).
...
and rate $\gamma > 0$, the scalar affinity decays exponentially:
...
pointer states $\{\vert\pi_k\rangle\}$ are the unique asymptotically
stable states of this evolution.
\end{proposition}

\begin{proof}
With $L_k = \sqrt{\gamma}\,\vert\pi_k\rangle\langle\pi_k\vert$,
we evaluate the dissipative part of Eq.~\eqref{eq:lindblad} for an
off-diagonal element $\rho_{ij} \equiv \langle\pi_i\vert\rho_S\vert\pi_j\rangle$,
$i \neq j$.
The gain term contributes
\begin{equation*}
  \sum_k \gamma\,
  \langle\pi_i\vert L_k\rho_S L_k^\dagger\vert\pi_j\rangle
  \;=\;
  \gamma\sum_k \delta_{ik}\,\rho_{kk}\,\delta_{kj}
  \;=\; 0
  \qquad (i \neq j).
\end{equation*}
The loss term contributes
\begin{equation*}
  \frac{\gamma}{2}\sum_k
  ...
  \;=\; \gamma\,\rho_{ij}.
\end{equation*}
Hence $\dot{\rho}_{ij} = -\gamma\,\rho_{ij}$, consistent with
Eq.~\eqref{eq:decoherence-decay} at uniform rate
$\Gamma_{ij} = \gamma$.
Integrating: $\rho_{ij}(t) = \rho_{ij}(0)\,e^{-\gamma t}$ for all
$i \neq j$.
Since every off-diagonal entry carries the same exponential factor,
\begin{equation*}
...
Diagonal elements are preserved by this evolution (the gain and loss
terms cancel on the diagonal), confirming that pointer states
$\vert\pi_k\rangle\langle\pi_k\vert$ are the asymptotically stable
...
The Cauchy--Schwarz equality $\lvert\rho_{ij}\rvert^2 = \rho_{ii}\rho_{jj}$
holds iff $\rho_S$ is a rank-one projector (pure state).
...
state $\rho_{SE}(0) = \vert\psi_0\rangle\langle\psi_0\vert$.
\begin{enumerate}
  ...
  \item[\emph{(iii)}] \emph{Coherence transfer.}
    Under pointer-basis dephasing, as $\bar{\alpha}(S,E;\,t)$ decreases,
    the off-diagonal coherences of $\rho_S$ are encoded in the growing
    distinguishability of the pointer-correlated environmental branches
    $\{\vert E_k(t)\rangle\}$, not annihilated.
    Specifically, if $\vert\psi(t)\rangle = \sum_k c_k\,
    \vert\pi_k\rangle\vert E_k(t)\rangle$, then
    \begin{equation}
    \label{eq:coherence-transfer}
      ...
      \qquad i \neq j,
    \end{equation}
    so that $\bar{\alpha}(t) \to 0$ as and only as
    $\langle E_j(t)\vert E_i(t)\rangle \to 0$ for all $i \neq j$.
\end{enumerate}
\end{proposition}
...
gives $S(\rho_S) = S(\rho_E)$ and
$S(\rho_{SE}) = 0$.
Hence $I(S\!:\!E) = S(\rho_S) + S(\rho_E) - S(\rho_{SE})
= 2\,S(\rho_S)$.
As decoherence increases the mixedness of $\rho_S$
(off-diagonal elements decay while populations are preserved by
pointer-basis dephasing), $S(\rho_S)$ is non-decreasing.

\emph{Part (iii).}
Write the evolved joint state as
$\vert\psi(t)\rangle = \sum_k c_k\,\vert\pi_k\rangle\vert E_k(t)\rangle$,
where $c_k = \langle\pi_k\vert\psi_0\rangle$ and $\{\vert E_k(t)\rangle\}$
are the time-evolved environmental branches (not necessarily orthonormal).
...
The Markovian approximation leading to Eq.~\eqref{eq:alpha-decay}
replaces $\langle E_j(t)\vert E_i(t)\rangle \approx e^{-\gamma t}$,
reflecting the exponential loss of overlap as the environmental
branches become distinguishable.
When $\langle E_j(t)\vert E_i(t)\rangle = 0$ for all $i \neq j$,
the joint state is a fully branched Schmidt decomposition and $S$
has acquired a definite pointer-state record in $E$; no coherence
has been destroyed --- it has been distributed into the
orthogonality structure of $\{\vert E_k\rangle\}$.
\end{proof}

\begin{remark}[Implications for quantum error correction]
Proposition~\ref{prop:conservation} has a direct bearing on the
design of the $\URA$ gate (Sec.~\ref{sec:ura}).
