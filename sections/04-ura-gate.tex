...
The \RCT~establishes that relational coupling between a system and its
environment is a continuous resource: the scalar affinity
$\bar\alpha(S,E)$ ranges over $[0,1]$ (Proposition~\ref{prop:correspondence})
and decays continuously under decoherence
(Proposition~\ref{prop:determination}), and the conservation law of
Proposition~\ref{prop:conservation} establishes that this coherence is
\emph{redistributed} into $S$-$E$ correlations rather than destroyed.
A two-qubit entangling gate that takes this resource picture seriously
should therefore expose a continuous coupling angle, allowing the
operation to be tuned to the coherence budget actually available in
hardware, rather than committing to a fixed entanglement capability as
conventional two-qubit gates do.
...
Heisenberg exchange interaction, established by Loss and
DiVincenzo~\cite{LossDiVincenzo1998} as a universal two-qubit primitive
for solid-state quantum computation and the natural exchange coupling
realized on superconducting platforms~\cite{Krantz2019};
it generates entanglement symmetrically across all three Pauli axes.
...
What the \RCT~contributes is the conceptual warrant for a continuously
...
hardware whose decoherence rate is non-negligible, the gate trades a
fraction of its entanglement capability for a proportionally shorter
operation time, accumulating less noise per gate
(Sec.~\ref{sec:sim}, Eq.~\eqref{eq:peff}).
This is the operational expression of
Proposition~\ref{prop:conservation}: the coherence ``spent'' by an
entangling operation is redistributed into $S$-$E$ correlations rather
than annihilated, and a smaller exchange angle spends less of the local
$\bar\alpha$ budget per gate.
Fixed maximally entangling gates have no analogous tunable;
Sec.~\ref{sec:sim} quantifies the resulting fidelity advantage of
$\URA(\theta < \pi/4)$ numerically.
