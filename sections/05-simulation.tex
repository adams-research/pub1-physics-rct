...
density-matrix simulation in Qiskit and Cirq under the corresponding
Kraus channels, applied per-qubit at the gate-time-scaled rate;
they agree with the closed-form depolarizing values to within a few
parts in $10^3$ and confirm that the gate-time advantage is robust
across the three dominant decoherence mechanisms in superconducting
hardware.
...
analytical values ($\Delta F = 3 p_{\rm base}/8$). Dephasing and
amplitude-damping rows: Qiskit/Cirq density-matrix simulation under
the corresponding per-qubit Kraus channels at the gate-time-scaled
rate. Positive values confirm that $\URA(\pi/8)$ outperforms the fixed
gates in all high-decoherence regimes tested.}
